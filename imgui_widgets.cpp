...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
	...
	window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, utils::GetColorWithAlpha(c::anim::active, style.Alpha), 30.f);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	if (is_multiline) // Open group before calling GetID() because groups tracks id created within their scope (including the scrollbar)
		BeginGroup();
	const ImGuiID id = window->GetID(label);
	const float w = GetContentRegionMax().x - 31;
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	...
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 32));
	...
	it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::label::active : hovered ? c::label::hovered : c::label::default, g.IO.DeltaTime * 6.f);
	...
	if (render_cursor || render_selection)
	{
		...
		{
			ImU32 col = GetColorU32(is_displaying_hint ? c::label::default.Value : c::label::active.Value);
		...
		if (render_cursor)
		{
			...
			if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
				draw_window->DrawList->AddLine(ImVec2(total_bb.Min.x + it_anim->second.slow, cursor_screen_rect.Min.y), ImVec2(total_bb.Min.x + it_anim->second.slow, cursor_screen_rect.GetBL().y), GetColorU32(c::label::active.Value));
	...
	GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w, frame_bb.Min.y + (32 - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label, label_end);
